...
#     if request.method == "POST":
#         form_commande = CommandeForm(request.POST)
#         produit_ids = request.POST.getlist("produit")  # ← Maintenant cohérent
#         quantites = request.POST.getlist("quantite")

#         # Debug: afficher ce qui est reçu
#         print(f"Produits: {produit_ids}")
#         print(f"Quantités: {quantites}")

#         items_data = []
#         for p, q in zip(produit_ids, quantites):
#             if p and q:  # Vérifier que les deux ne sont pas vides
#                 try:
#                     items_data.append((int(p), int(q)))
#                 except ValueError:
#                     continue
...
#             messages.error(request, f"Erreur dans le formulaire: {form_commande.errors}")
        
#         if not items_data:
#             messages.error(request, "Veuillez sélectionner au moins un produit avec quantité.")
...
#                     for produit_id, quantite in items_data:
...
#     item = get_object_or_404(CommandeItem, id=item_id)
#     commande = item.commande
#     produit = item.boisson # Récupération de la boisson
#     quantite_supprimee = item.quantite # Récupération de la quantité avant suppression
...
#     # REMISE EN STOCK DE LA QUANTITÉ
#     # *********************************
#     produit.stock_actuel += quantite_supprimee
#     produit.save(update_fields=['stock_actuel'])
...
#     # Recalcul du montant total
#     total = sum([i.quantite * i.produit.prix_unitaire for i in commande.items.all()])
#     commande.montant_total = total
#     commande.save()
...
#         "commande_total": float(total), 
#         "deleted": True,
#         "new_stock": produit.stock_actuel, # Optionnel : retourner le nouveau stock
#         "produit_nom": produit.nom, # Optionnel : retourner le nom
#     })
...
#             # Mise à jour du stock
#             produit.stock_actuel += quantite_remise
#             produit.save(update_fields=['stock_actuel'])
...
# @login_required
# @session_required # <--- APPLICATION DU CONTRÔLE DE SESSION
# def ajouter_produit(request, commande_id):
#     commande = get_object_or_404(Commande, id=commande_id)
#     produit_id = request.POST.get("produit_id")
#     quantite = int(request.POST.get("quantite", 1))

#     if not produit_id:
#         return JsonResponse({"success": False, "message": "Veuillez choisir un produit."})

#     produit = get_object_or_404(Produit, id=produit_id)

#     # Vérification du stock disponible
#     if produit.stock_actuel < quantite:
#         return JsonResponse({"success": False, "message": f"Stock insuffisant. Reste: {produit.stock_actuel}"})
...
#     if not created:
#         # Si l'item existe, vérifions si l'ajout dépasse le stock
#         if produit.stock_actuel < item.quantite + quantite:
#             return JsonResponse({"success": False, "message": f"Stock insuffisant. Reste: {produit.stock_actuel}"})
...
#     # Recalcul du montant total de la commande
#     total = sum([i.quantite * i.produit.prix_vente for i in commande.items.all()])
#     commande.montant_total = total
#     commande.save()
...
#         "commande_total": float(total),
#         "item_id": item.id,
#         "new_stock": produit.stock_actuel, # Retourner le nouveau stock
...
# def update_item_quantity(request, item_id):
#     item = get_object_or_404(CommandeItem, pk=item_id)
#     produit = item.produit # On récupère la boisson associée
#     action = request.POST.get("action")
...
#             # On ne peut pas incrémenter si le stock est déjà à 0
#             return JsonResponse({"success": False, "error": f"Stock épuisé pour {produit.nom}"})
...
#             total = sum([i.montant_produit for i in commande.items.all()])
#             return JsonResponse({
#                 "success": True,
#                 "deleted": True,
#                 "item_id": item_id,
...
#         "commande_id": commande.id,
#         "quantite": item.quantite,
#         "item_montant": float(item.montant_produit),
#         "commande_total": float(total),
#         "new_stock": produit.stock_actuel, # Retourner le nouveau stock
#         "produit_nom": produit.nom,
#     })

###################### Prise en compte des produits non denombrables ##############
    ...
    if request.method == "POST":
        ...
        if form_commande.is_valid() and items_data:
            ...
            for produit_id, quantite in items_data:
                ...
                # CORRECTION : Vérification du stock uniquement pour les produits dénombrables
                if produit.nature == 'denombrable':
                    stock_actuel = produit.stock_actuel if produit.stock_actuel is not None else 0
                    if stock_actuel < quantite:
                        stock_errors.append(f"{produit.nom} (Demandé: {quantite}, Stock: {stock_actuel})")
                    else:
                        items_to_create.append({'produit': produit, 'quantite': quantite, 'denombrable': True})
                else:
                    # Pour les produits non dénombrables, pas de vérification de stock
                    items_to_create.append({'produit': produit, 'quantite': quantite, 'denombrable': False})

            if stock_errors:
                error_message = "Erreur: Stock insuffisant pour les produits suivants : " + ", ".join(stock_errors)
                messages.error(request, error_message)
                return redirect("commandes:ajouter_commande")
            ...
            try:
                with transaction.atomic():
                    ...
                    for item_data in items_to_create:
                        ...
                        quantite = item_data['quantite']
                        est_denombrable = item_data['denombrable']
                        
                        # CORRECTION : Décrémenter le stock seulement pour les produits dénombrables
                        if est_denombrable:
                            produit.stock_actuel -= quantite
                            produit.save(update_fields=['stock_actuel'])
...
def ajouter_produit(request, commande_id):
    ...
    if produit.nature == 'denombrable':
        stock_actuel = produit.stock_actuel if produit.stock_actuel is not None else 0
        if stock_actuel < quantite:
            return JsonResponse({"success": False, "message": f"Stock insuffisant. Reste: {stock_actuel}"})
    ...
    if not created and produit.nature == 'denombrable':
        # Si l'item existe et le produit est dénombrable, vérifier le stock
        stock_actuel = produit.stock_actuel if produit.stock_actuel is not None else 0
        if stock_actuel < item.quantite + quantite:
            return JsonResponse({"success": False, "message": f"Stock insuffisant. Reste: {stock_actuel}"})
    ...
    # CORRECTION : Décrémentation du stock seulement pour les produits dénombrables
    if produit.nature == 'denombrable' and produit.stock_actuel is not None:
        produit.stock_actuel -= quantite
        produit.save(update_fields=['stock_actuel'])
    ...
    return JsonResponse({
        ...
        "commande_total": float(total),
        "item_id": item.id,
        "new_stock": produit.stock_actuel, # Retourner le nouveau stock (peut être None)
        "produit_nom": produit.nom,
        "produit_nature": produit.nature, # Ajouter la nature pour le frontend
    })
...
def update_item_quantity(request, item_id):
    ...
    if action == "increment":
        ...
        if item.quantite == 1:
            ...
            # CORRECTION : Remise en stock seulement pour les produits dénombrables
            if produit.nature == 'denombrable' and produit.stock_actuel is not None:
                produit.stock_actuel += 1
                produit.save(update_fields=['stock_actuel'])
            ...
            return JsonResponse({
                ...
                "commande_id": commande.id,
                "commande_total": float(total),
                "new_stock": produit.stock_actuel,
                "produit_nom": produit.nom,
            })
        else:
            item.quantite -= 1
...
def supprimer_item(request, item_id):
    item = get_object_or_404(CommandeItem, id=item_id)
    commande = item.commande
    produit = item.produit  # CORRECTION : produit au lieu de boisson
    quantite_supprimee = item.quantite
    ...
    # Recalcul du montant total
    total = sum([i.quantite * i.produit.prix_vente for i in commande.items.all()])  # CORRECTION : prix_vente au lieu de prix_unitaire
    commande.montant_total = total
    commande.save()
